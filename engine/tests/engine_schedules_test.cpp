...
TEST_F(EngineSchedulesTest, VwapOrderFollowsWeights) {
    ...
    EXPECT_DOUBLE_EQ(engine.step(kSec).fills.at(0).qty, 3.0);
...
TEST_F(EngineSchedulesTest, AlmgrenChrissOrderFrontLoads) {
    ...
    ASSERT_TRUE(engine.submit({"ac", Side::Buy, 1.0, 0, 4 * kSec, 4},
                    ...
                    .accepted);
    EXPECT_NEAR(engine.step(0).fills.at(0).qty, 1.0 - std::sinh(1.5) / std::sinh(2.0), 1e-9);
...
TEST_F(EngineSchedulesTest, PovOrderHaltsAtDeadline) {
    ...
    ASSERT_TRUE(engine.apply_trades({{100.0, 5.0}}));
    EXPECT_DOUBLE_EQ(engine.step(kSec).fills.at(0).qty, 1.0);
    EXPECT_TRUE(engine.step(2 * kSec).fills.empty());
...
TEST_F(EngineSchedulesTest, OrderCompletingOnItsDeadlineStepIsNotHalted) {
    ...
    EXPECT_EQ(engine.step(4 * kSec).fills.size(), 1u);
